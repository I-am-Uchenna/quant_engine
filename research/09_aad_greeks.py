...
# single parameter vector theta = [z..., sigma] so one reverse pass gives all greeks
def pt(theta): return price(theta[:-1], theta[-1])
theta0=np.concatenate([z0,[sig0]])
V=pt(theta0); print(f"AAD pricer value: ${V:,.2f}  (policy-fixed, real FRED curve, sigma={sig0*1e4:.1f}bp)")
t0=time.time(); g=grad(pt)(theta0); t_aad=time.time()-t0
gz=g[:-1]; gs=g[-1]
h=1e-6
t1=time.time()
gz_fd=np.array([(pt(theta0+h*np.eye(len(theta0))[i])-V)/h for i in range(len(z0))]); gs_fd=(pt(theta0+h*np.eye(len(theta0))[-1])-V)/h
t_fd=time.time()-t1
print(f"\nVega:  AAD ${gs*1e-4:,.2f}/bp   bump ${gs_fd*1e-4:,.2f}/bp   rel.err {abs(gs-gs_fd)/abs(gs):.2e}")
print("\nKey-rate DV01 (AAD vs bump, per 1bp):")
print(f"   {'tenor':>6} {'AAD':>10} {'bump':>10} {'rel.err':>9}")
for i,t in enumerate(T):
    if abs(gz[i])*1e-4<0.5 and abs(gz_fd[i])*1e-4<0.5: continue
    re=abs(gz[i]-gz_fd[i])/(abs(gz[i])+1e-9)
    print(f"   {t:>5.3g}y {gz[i]*1e-4:>10,.2f} {gz_fd[i]*1e-4:>10,.2f} {re:>9.1e}")
print(f"\nCost: AAD whole gradient ({len(theta0)} sensitivities) in 1 reverse pass = {t_aad:.3f}s")
print(f"      bump-and-revalue = {len(theta0)} forward passes = {t_fd:.3f}s  ->  AAD speedup {t_fd/t_aad:.1f}x")
